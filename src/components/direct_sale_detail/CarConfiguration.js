...
class CarConfigurationState {
  @observable carConfigurationBrandName = null
  @observable carConfigurationData = []
  @observable currentCarConfigurationIndex = 0

  async fetchCarConfigurationData(currentCarTypeId){
    appState.I_M_Fucking_Loading()

    const search = '?source=101&time=123456&mark=ds'
    let url = `${GLOBAL_API_DOMAIN}/ware/car/310000/car-type/${currentCarTypeId}/config${search}`
    const Response = await fetch(url, this.options)
    const ResponseOK = await Response.ok
    const ResponseJSON = await Response.json()
    if (ResponseOK) {
      if(ResponseJSON.status == 1){
        this.carConfigurationBrandName = ResponseJSON.data.brandAndType
        this.carConfigurationData = ResponseJSON.data.crawlerCarConfigureList
      }
    }

    appState.I_Have_Done_Fucking_Loading()
  }
  switchCarConfigurationIndex(offset){
    const indexWithOffset = this.currentCarConfigurationIndex + offset
    console.log(indexWithOffset)
    if(indexWithOffset >= 0 && indexWithOffset <= this.carConfigurationData.length - 1){
      this.currentCarConfigurationIndex = indexWithOffset
    }
  }
}

@observer
class CarConfiguration extends React.Component {
  constructor(props){
    super(props)
  ...
  async componentDidMount(){
    const { currentCarModelOptionIndex, currentCarTypeId } = this.props.popupDirectSaleOptionsState
    await this.carConfigurationState.fetchCarConfigurationData(currentCarTypeId)

    this.carConfigurationState.currentCarConfigurationIndex = currentCarModelOptionIndex
  }
  render(){
    const { currentCarTypeId } = this.props.popupDirectSaleOptionsState
    const { currentCarConfigurationIndex, carConfigurationData, switchCarConfigurationIndex } = this.carConfigurationState

    if(carConfigurationData.length === 0) {
      return null
    }
    ...
    const {
      ...
      seatConfiguration,
      status,
      wheelBrake } = carConfigurationData[currentCarConfigurationIndex]

    let originalModelNameArray = ['', '', '']

    if(currentCarConfigurationIndex > 0){
      originalModelNameArray[0] = carConfigurationData[currentCarConfigurationIndex - 1].modelInformation.originalModelName || ''
    }

    originalModelNameArray[1] = carConfigurationData[currentCarConfigurationIndex].modelInformation.originalModelName || ''

    if(currentCarConfigurationIndex < carConfigurationData.length - 1){
      originalModelNameArray[2] = carConfigurationData[currentCarConfigurationIndex + 1].modelInformation.originalModelName || ''
    }

    return(
      <div className="configuration">
        <div className="carName">
          <button className="prev" onClick={switchCarConfigurationIndex.bind(this.carConfigurationState, -1)}>&lt;</button>
          <ul>
            {
              originalModelNameArray.map(
                (originalModelName, index) => <li>{ originalModelName }</li>
              )
            }
          </ul>
          <button className="next" onClick={switchCarConfigurationIndex.bind(this.carConfigurationState, 1)}>&gt;</button>
        </div>
        <ul>
          <li>
            <div>
